rm -fR classes/* &&
  ...
  find api/src/main/java/io/github/tivrfoa/mapresultset/ -name "*.java" > mapsources.txt &&
  echo '--- Compiling API:' &&
  echo '--------------' && cat mapsources.txt && echo '---------------' &&
  javac -d classes/ @mapsources.txt &&

  find processor/src/main/java/io/github/tivrfoa/mapresultset/ -name "*.java" > mapsources.txt &&
...
## -proc:only does not work. It tries to compile anyways :(
  javac -verbose  -cp classes/ -Xlint:processing -proc:only -implicit:none  -processor io.github.tivrfoa.mapresultset.MappingProcessor \
...
## https://stackoverflow.com/questions/72795137/first-mvn-compile-fails-to-find-generated-sources
  ...
  javac -cp classes/:lib/junit-jupiter-api-5.6.2.jar:lib/apiguardian-api-1.1.0.jar \
    -d classes/ @sources.txt integrationtest/src/test/java/io/github/tivrfoa/mapresultset/TestProcessor.java &&
  ...
  java -cp classes/:lib/mysql-connector-java-8.0.29.jar:lib/junit-jupiter-5.6.2.jar:lib/junit-jupiter-api-5.6.2.jar:lib/opentest4j-1.2.0.jar \
    -ea io.github.tivrfoa.mapresultset.TestProcessor
